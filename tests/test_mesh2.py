...
def test_ref():
    ...
    with create_sharding_mesh() as sharding_mesh:
        ...
        for sort_ibin in [True, False]:
            bin = BinMesh2SpectrumPoles(mattrs, edges={'step': 4 * mattrs.kfun.min()}, ells=ells, sort_ibin=sort_ibin)
            result = {}
        ...
        for sort_ibin in [True, False]:
            ...
            for basis in [None, 'bessel']:
                bin = BinMesh2CorrelationPoles(mattrs, edges={'step': 4 * mattrs.cellsize.min()}, ells=ells, sort_ibin=sort_ibin)
                for los in ['x', 'firstpoint', 'endpoint']:
